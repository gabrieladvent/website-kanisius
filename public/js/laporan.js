function updateNamaSekolahOptions() {
    ...
    var selectedOptions = document.querySelectorAll(
        ".option-" + selectedTingkatan.toLowerCase()
    );
...
const radioZonasi = document.getElementById("zonasi");
const radioJS = document.getElementById("jumlah_siswa");
...
function enableDropdowns() {
    const selectedRadio = document.querySelector(
        'input[name="laporanType"]:checked'
    );
    if (selectedRadio.id === "penghasilan") {
        ...
        document.getElementById("namaSekolah").disabled = false;
        document.getElementById("tingkatan-tk").removeAttribute("disabled");
    } else if (selectedRadio.id === "zonasi") {
        document.getElementById("tingkatan").disabled = false;
        document.getElementById("namaSekolah").disabled = false;
        document.getElementById("tingkatan-tk").setAttribute("disabled", true);
    }else if (selectedRadio.id === "jumlah_siswa") {
        document.getElementById("tingkatan").disabled = false;
        document.getElementById("namaSekolah").disabled = false;
        document.getElementById("tingkatan-tk").disabled = false;
}
}
// Function to disable "Spesifik kelas" radio buttons
function disableSpesifikKelas() {
    const spesifikKelasRadios = formContainer.querySelectorAll(
        'input[name="detailKelas"]'
    );
...
radioZonasi.addEventListener("change", function () {
    disableAllDropdowns();
    enableDropdowns();
    disableSpesifikKelas();
});
radioJS.addEventListener("change", function () {
    disableAllDropdowns();
    enableDropdowns();
    disableSpesifikKelas();
});
